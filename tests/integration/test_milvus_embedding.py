...
def test_embedding_sync_utility_mock_mode(
    mock_neo4j_driver: MockNeo4jDriver, sample_embedding: np.ndarray
) -> None:
    """Test embedding sync utility in mock mode (no Milvus required).

    This test verifies the embedding sync utility can store and verify
    embeddings in Neo4j without requiring a live Milvus instance.
    """
    # Create sync utility in mock mode
    sync_util = EmbeddingSyncUtility(neo4j_driver=mock_neo4j_driver)
    ...
    # Verify UUID format
    assert embedding_id
    uuid.UUID(embedding_id)  # This will raise if not a valid UUID
    ...
    with mock_neo4j_driver.session() as session:
        ...
        assert embedding_data["embedding_id"] == embedding_id
        assert embedding_data["text"] == text
        assert embedding_data["metadata"] == metadata
        assert embedding_data["collection"] == "test"


def test_multiple_embeddings_mock_mode(
    mock_neo4j_driver: MockNeo4jDriver, sample_embedding: np.ndarray
) -> None:
    """Test storing multiple embeddings in mock mode.

    This test verifies that multiple embeddings can be stored and tracked
    independently.
    """
    sync_util = EmbeddingSyncUtility(neo4j_driver=mock_neo4j_driver)
    ...
    for text in texts:
        ...
        embedding_id = sync_util.store_embedding(
            text, embedding, metadata={"index": len(embedding_ids)}
        )
    ...
    # Verify count in mock storage
    assert len(mock_neo4j_driver.embeddings) == len(texts)
